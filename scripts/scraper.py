...
HEADERS = {"User-Agent": "ProductAlertSpain/2.3 (+https://github.com/progeekts/Product-Alert)", "Accept-Language": "es-ES,es;q=0.9,en;q=0.5"}
...
def parse_generic(source):
 ...
 for a in soup.find_all("a",href=True):
  ...
  alerts.append({"id":stable_id(source["agency"],title,href),"source_id":source["id"],"title":title,"agency":source["agency"],"category":source["category"],"date":date,"link":href,"description":title,"reference":reference,"risk":infer_risk(title),"severity":infer_severity(title),"status":"Publicada","lotes":extract_lots(raw),"barcodes":[]})
 return alerts

def parse_aemps_medicines(source):
 # AEMPS publica estas retiradas como entradas individuales de la categoría
 # Medicamentos Uso Humano. El listado principal usa carga dinámica y por eso
 # no es apropiado para el parser genérico.
 soup=get_soup(source["url"]); alerts=[]; seen=set()
 for heading in soup.find_all(["h2","h3"]):
  ...
  title=clean_title(clean_text(a))
  if len(title)<12 or "información adicional" in title.lower(): continue
  container=heading.parent
  context=clean_text(container)
  if not (re.search(r"R[_ -]?\d{1,3}/20\d{2}",context,re.I) or "nº alerta" in context.lower()): continue
  reference=extract_reference(context)
  date=normalize_date(context)
  seen.add(href)
  alerts.append({"id":stable_id(source["agency"],title,href),"source_id":source["id"],"title":title,"agency":"AEMPS","category":"Medicamentos","date":date,"link":href,"description":title,"reference":reference,"risk":"Defecto de calidad","severity":"informativa","status":"Publicada","lotes":extract_lots(context),"barcodes":[]})
 return alerts

def parse_source(source):
 return parse_aemps_medicines(source) if source.get("parser")=="aemps_medicines" else parse_generic(source)
def dedupe(alerts):
 by_key={}
 for x in alerts:
...
def previous_slice(previous,source):
 ...
 if len(same)==1:return [x for x in previous.get("alerts",[]) if x.get("agency")==source["agency"] and x.get("category")==source["category"]]
 return []
def main():
 ...
 for source in SOURCES:
  ...
  try:
   ...
   collected.extend(items); source_status.append({"id":source["id"],"agency":source["agency"],"category":source["category"],"url":source["url"],"ok":True,"stale":False,"items":len(items),"checked_at":checked,"last_success":checked})
   print(f"[{source['id']}] {len(items)} registros detectados")
  except Exception as exc:
   cached=previous_slice(previous,source); collected.extend(cached); old=next((s for s in previous.get("meta",{}).get("sources",[]) if s.get("id")==source["id"] or s.get("url")==source["url"]),{})
   source_status.append({"id":source["id"],"agency":source["agency"],"category":source["category"],"url":source["url"],"ok":False,"stale":True,"items":len(cached),"checked_at":checked,"last_success":old.get("last_success"),"error":str(exc)[:250]})
   print(f"[{source['id']}] ERROR; se conservan {len(cached)} registros previos: {exc}")
 alerts=dedupe(collected); alerts.sort(key=lambda x:(x.get("date",""),x.get("title","")),reverse=True)
 payload={"meta":{"last_checked":datetime.now(timezone.utc).isoformat(),"sources":source_status,"count":len(alerts),"scope":"España","version":5,"note":"Información agregada desde fuentes oficiales. Cada fuente se controla de forma independiente; ante un fallo se conserva su último conjunto conocido cuando está disponible y se marca como desactualizado. La publicación original prevalece siempre."},"alerts":alerts}
